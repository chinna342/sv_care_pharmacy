...
async function fixTramadol() {
  ...
  // Find tramadol in dataset
  const item = data.find(i => {
    const conv = (i.conversations || []).map(c => c.content || '').join(' ').toLowerCase();
    return (conv.includes('tramadol') || conv.includes('ultram') || conv.includes('paracetamol (325mg) + tramadol (37.5mg)')) && i.image;
  });

  if (item) {
    console.log('Found real Tramadol photograph:', item.image);
...
fixTramadol().catch(console.error);
