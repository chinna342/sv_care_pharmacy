...
def _get_smtp_connection():
    ...
    if not user or not password:
        logger.warning("[GMAIL SMTP] Missing GMAIL_USER or GMAIL_APP_PASSWORD in environment")
    ...
    try:
        ...
        logger.error(f"[GMAIL SMTP CONNECT ERROR on port {port}]: {e}")
        # Fallback to STARTTLS (587) if SSL (465) fails
        try:
            fallback_server = smtplib.SMTP("smtp.gmail.com", 587, timeout=15)
            fallback_server.starttls()
            fallback_server.login(user, password)
            return fallback_server, user, password
        except Exception as e2:
            logger.error(f"[GMAIL SMTP 587 FALLBACK ERROR]: {e2}")
            raise e
...
def send_otp_email(to_email: str, otp: str, name: str = "Valued Patient") -> dict:
    ...
    Includes both plain text and HTML multipart for optimal deliverability (avoids Spam folder).
    """
    ...
    try:
        ...
        if server and sender_email:
            ...
            # 1. Plain text fallback (Crucial for Spam prevention in Gmail)
            text_body = f"""SV CARE PHARMACY - Login Verification
Hello {name},
...
<head>
  ...
  <title>SV Care Verification Code</title>
</head>
...
"""
            ...
            raise ValueError("Gmail credentials missing in backend environment (.env).")
...
def send_order_confirmation_email(to_email: str, order_number: str, total_amount: float, name: str = "Customer") -> dict:
    ...
    try:
        ...
        if server and sender_email:
            ...
            text_body = f"""SV CARE PHARMACY - Order Confirmation #{order_number}
Hello {name},

Thank you for choosing SV Care. We have received your order #{order_number} with total amount ₹{total_amount:.2f}.
Our registered pharmacist is currently verifying your items for fast 15-30m cold-chain delivery.

SV Care Global Pharmacy & Clinical Suite
Telangana Lic: TS/HYD/2026/8942-R
"""
            msg.attach(MIMEText(text_body, "plain", "utf-8"))
