...
// In-memory shared orders store across warm serverless invocations
...
export default async function handler(req, res) {
  ...
  // 1. GET ALL ORDERS (For Pharmacist, Admin, Delivery & Customer)
  if (req.method === "GET") {
    return res.status(200).json(global.ordersStorage);
  }

  // 2. CREATE NEW ORDER (Placed by any customer on any device)
  if (req.method === "POST") {
    try {
      const data = req.body || {};
      ...
      // Add to front of cloud store
      global.ordersStorage.unshift(newOrder);

      // Send Email notification to Store / Admin / Customer if Gmail configured
      try {
  ...
  // 3. UPDATE ORDER STATUS (e.g. ACCEPT, PACK, DISPATCH, DELIVER)
  if (req.method === "PUT") {
    try {
